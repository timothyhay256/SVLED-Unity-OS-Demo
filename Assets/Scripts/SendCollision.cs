...
public class SendCollision : MonoBehaviour
{
    ...
    public readonly LinkedList<CollisionInfo> collisionStack = new();
    public readonly HashSet<Collider> activeColliders = new();
    ...
    private void SendClearLoop()
    {
        byte[] buffer = Encoding.ASCII.GetBytes("CLEAR");
        client.Send(buffer, buffer.Length, ep);
    }
    ...
    {
        if (!useCheckSphereInstead && !col.CompareTag("LED"))
        {
            ...
            else
            {
                ...
                collisionStack.AddFirst(new CollisionInfo { collider = col, color = color });
                activeColliders.Add(col);
                if (brushColor == null) // Only apply if no brush is overriding
                    ApplyColor(color);
    ...
    private void OnTriggerExit(Collider col)
    {
        if (!useCheckSphereInstead && activeColliders.Contains(col))
        {
            ...
            if (col.CompareTag("Brush"))
            {
                brushColor = null;
                // Re-apply top of stack if available
                if (collisionStack.Count > 0)
                {
                    ApplyColor(collisionStack.First.Value.color);
                    collisionUpdated = true;
            ...
            {
                // Only update color if Brush isn't overriding
                if (collisionStack.Count > 0)
                {
                    ApplyColor(collisionStack.First.Value.color);
                    collisionUpdated = true;
